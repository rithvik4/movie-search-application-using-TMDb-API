...
const Thumbnail = forwardRef(({ result }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className="group p-2 cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 hove:z-50"
    >
      <Image
        layout="responsive"
        src={
      ...
      <div className="p-2">
        ...
        <p className="flex item-center opacity-0 group-hover:opacity-100">
          ...
          <ThumbUpIcon className="h-5 max-w-2 mr-1" /> {result.vote_count}
        </p>
      </div>
    </div>
  );
